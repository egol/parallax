...
for _ in range($attempts):
    data=json.load(urllib.request.urlopen(url, timeout=3))['data']
    if $condition:
...
echo "real-model smoke uses a tiny CPU model and can still take time on a cold cache or slow network"
compose up -d --build
...
compose exec -T worker1 sh -lc "PARALLAX_TEST_RUNTIME=transformers nohup parallax join -u --scheduler-addr $scheduler_addr --host 0.0.0.0 --port 3101 --tcp-port 4101 --udp-port 5101 >/tmp/parallax-join.log 2>&1 &"
compose exec -T worker2 sh -lc "PARALLAX_TEST_RUNTIME=transformers nohup parallax join -u --scheduler-addr $scheduler_addr --host 0.0.0.0 --port 3102 --tcp-port 4102 --udp-port 5102 >/tmp/parallax-join.log 2>&1 &"
...
wait_status "data.get('initialized') is True" 90 >/dev/null
...
python3 - <<'PY' "$chat_response" "$MODEL_NAME"
import json
import sys
